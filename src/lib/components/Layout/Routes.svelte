<script lang="ts">
	...
	const showLoginOption = $derived(!$page.data.session && !$page.url.pathname.startsWith('/auth'));
...
<li>
	<a
		...
		class:active={route.id === '/'}>Inicio</a
	>
</li>
<li>
	<a
		href="/participacion"
		class="btn-link text-black no-underline hover:text-primary"
		class:active={route.id?.startsWith('/participacion') ||
			route.id?.startsWith('/propuestas') ||
			route.id?.startsWith('/actividades')}>Participación</a
	>
</li>
<li>
	<a
		href="/comunidades"
		class="btn-link text-black no-underline hover:text-primary"
		class:active={route.id?.startsWith('/comunidades')}>Comunidades</a
	>
</li>
{#if showLoginOption}
	<li class="flex flex-col justify-center">
		<a href="/auth/login" class="text-black no-underline hover:text-primary hover:border-b-[1px] hover:border-b-primary hover:border-b-solid flex items-center">
			{#if true}
...
{:else if userIsLogged}
    <li class="flex flex-col justify-center">
        <a href="/usuario" aria-label="Perfil del usuario" class="text-primary no-underline text-primary border-b-[1px] border-b-primary hover:border-b-solid flex items-center">
            <svg xmlns="http://www.w3.org/2000/svg" width="35" height="30" viewBox="0 0 24 24"
				><path
					fill="currentColor"
					fill-rule="evenodd"
...
{:else}
	<li class="flex flex-col justify-center">
		<a href="/auth/login" class="text-primary no-underline text-primary border-b-[1px] border-b-primary hover:border-b-solid flex items-center">
			{#if true}
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M4 12h16m0 0l-6-6m6 6l-6 6"/></svg>
			{/if}
			<svg xmlns="http://www.w3.org/2000/svg" width="35" height="30" viewBox="0 0 24 24"
...
<style>
	.active {
		...
		@apply btn-link cursor-pointer text-primary;
	}
</style>
